/*! angular-svg-round-progressbar 2015-09-03 */
"use strict";
!function () {
    ...
    }, a.updateState = function (a, b, d, e, f, g) {
        if (!f)return e;
        var h = a >= b ? b - 1e-5 : a, i = g ? 180 : 359.9999, j = 0 === b ? 0 : h / b * i, k = f / 2, l = c(k, k, d, j), m = c(k, k, d, 0), n = 180 >= j ? "0" : "1", o = ["M", l.x, l.y, "A", d, d, 0, n, 0, m.x, m.y].join(" ");
        return e.attr("d", o)
    }, a.animations = {
    ...
    return b.isSupported ? angular.extend(d, {
        link: function (e, f) {
            var g, h = f.find("svg").eq(0), i = h.find("path").eq(0), j = h.find("circle").eq(0), k = angular.copy(c), l = function () {
                var a = k.semi, b = k.responsive, c = parseInt(k.radius) || 0, d = parseInt(k.stroke), e = 2 * c, g = c - d / 2;
                h.css({
                    top: 0,
                    left: 0,
                    position: b ? "absolute" : "static",
                    width: b ? "100%" : e + "px",
                    height: b ? "100%" : (a ? c : e) + "px",
                    overflow: "hidden"
                }), h[0].setAttribute("viewBox", "0 0 " + e + " " + (a ? c : e)), f.css({
                    width: b ? "100%" : "auto",
                    position: "relative",
                    "padding-bottom": b ? a ? "50%" : "100%" : 0
                }), i.css({
                    stroke: k.color,
                    "stroke-width": d,
                    "stroke-linecap": k.rounded ? "round" : "butt"
                }), a ? i.attr("transform", k.clockwise ? "translate(0," + e + ") rotate(-90)" : "translate(" + e + ", " + e + ") rotate(90) scale(-1, 1)") : i.attr("transform", k.clockwise ? "" : "scale(-1, 1) translate(" + -e + " 0)"), j.attr({
                    cx: c,
                    cy: c,
                    r: g >= 0 ? g : 0
                }).css({stroke: k.bgcolor, "stroke-width": d})
            }, m = function (c, d) {
                var e = b.toNumber(k.max || 0), f = c > e ? e : 0 > c || !c ? 0 : c, h = d === f || 0 > d ? 0 : d || 0, j = f - h, l = b.animations[k.animation], m = new Date, n = parseInt(k.duration) || 0, o = c > e && d > e || 0 > c && 0 > d || 25 > n, p = k.radius, q = p - k.stroke / 2, r = 2 * p, s = k.semi;
                o ? b.updateState(f, e, q, i, r, s) : (a.cancelAnimationFrame(g), function t() {
                    var c = new Date - m;
                    b.updateState(l(c, h, j, n), e, q, i, r, s), n > c && (g = a.requestAnimationFrame(t))
                }())
            };
            e.$watchCollection("[" + Object.keys(d.scope).join(",") + "]", function (a, c, d) {
                angular.forEach(d, function (a, b) {
                    b.indexOf("$") && d !== a && angular.isDefined(a) && (k[b] = a)
                }), l(), m(b.toNumber(a[0]), b.toNumber(c[0]))
            })
        },
        template: ['<div class="round-progress-wrapper">', '<svg class="round-progress" xmlns="http://www.w3.org/2000/svg">', '<circle fill="none"/>', '<path fill="none"/>', "<g ng-transclude></g>", "</svg>", "</div>"].join("\n")
